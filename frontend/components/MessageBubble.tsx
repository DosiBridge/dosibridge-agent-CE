...
export default function MessageBubble({ message }: MessageBubbleProps) {
    ...
    return (
        <div className={`group flex gap-2 sm:gap-3 md:gap-4 mb-3 sm:mb-4 md:mb-6 message-enter px-1 sm:px-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
            {!isUser && (
            ...
            <div className={`flex flex-col max-w-[85%] xs:max-w-[90%] sm:max-w-[85%] md:max-w-[75%] lg:max-w-[70%] ${isUser ? 'items-end' : 'items-start'}`}>
                <div className="relative w-full">
                    <div
                        className={`rounded-2xl px-3 py-2 sm:px-3.5 sm:py-2.5 md:px-4 md:py-3 shadow-sm ${isUser
            ...
            {isUser && (
                <div className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-full bg-[#10a37f] flex items-center justify-center shadow-md">
